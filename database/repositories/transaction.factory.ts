...
export abstract class ITransactionRunner {
  ...
  abstract transactionManager: EntityManager;
}
...
class TransactionRunner implements ITransactionRunner {
  ...
  async rollbackTransaction(): Promise<void> {
    if (this.hasTransactionDestroyed || !this.queryRunner.isTransactionActive) {
      return;
    }
...
@Injectable()
export default class DbTransactionFactory {
